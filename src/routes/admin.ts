...
function shouldShowEntryTypeSummary(event: any) {
...
async function getAdminGroup(event: any) {
...
function formatArgentinaDateTime(value?: Date | string) {
  if (!value) return "";
...
router.get("/:slug/admin", requireAdmin, async (req: Request, res: Response) => {
  ...
  const filteredEventIds = selectedEventId && groupEventIds.some((id: any) => String(id) === selectedEventId)
    ? [selectedEventId]
    : groupEventIds;
  const rsvpQuery: any = { event_id: { $in: filteredEventIds } };
...
router.get("/:slug/admin/export.xlsx", requireAdmin, async (req: Request, res: Response) => {
  const slug = Array.isArray(req.params.slug) ? req.params.slug[0] : req.params.slug;
  const event = await getEventBySlug(slug);
...
router.post("/:slug/admin/rsvps/:rsvpId/delete", requireAdmin, async (req: Request, res: Response) => {
  const slug = Array.isArray(req.params.slug) ? req.params.slug[0] : req.params.slug;
  const event = await getEventBySlug(slug);
  if (!event) return res.status(404).render("errors/404", { url: req.originalUrl });

  const { groupEventIds } = await getAdminGroup(event);
  await Rsvp.deleteOne({ _id: req.params.rsvpId, event_id: { $in: groupEventIds } });
